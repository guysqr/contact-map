<template>
  <div class="map-panel">
    <div id="ControlPanel" class="control-panel">
      ...
      <dynamic-select
        :options="groupTypesArray"
        v-model="selectedGroupType"
        @input="showPolygonsInView(true)"
        option-value="value"
        option-text="label"
    ...
    >
      ...
      <l-geo-json
        v-for="g in geojsons"
        v-bind:key="g.id"
        @mouseover="showGeojsonPopup(g)"
        :geojson="g.geojson"
        :optionsStyle="g.style"
      ></l-geo-json>
      ...
      <l-control position="bottomright" style="z-index: 760">
        ...
        <vue-dropzone
          ...
          @vdropzone-success="handleData"
        ></vue-dropzone>
      </l-control>
      <l-draw-toolbar position="topleft" />
      <l-polyline
        v-for="p in filteredTrackData"
        v-bind:key="p.id"
        :lat-lngs="p.points"
        :color="p.style.color"
        ref="userTracks"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      url:
        "https://api.mapbox.com/styles/v1/mapbox/streets-v11/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiZ3V5bW9ydG9uIiwiYSI6ImNrOHkwNmg3bzAwMzkzZ3RibG9wem43N20ifQ.Lgs-FlpaE3S61_eGyTCEsQ",
      attribution:
  ...
  methods: {
    ...
    handleData(file, response) {
      ...
      for (var date in response) {
        ...
        for (var i = 0; i < response[date].length; i++) {
          ...
          response[date][i].start = formatDatetime(
            response[date][i].start,
            true
          );
    ...
    showGeojsonPopup(geojson) {
      ...
      this.geoJsonPopup = L.popup({ closeButton: false, autoPan: false, offset: [0,-20] })
    ...
    refreshGeojsonPopup() {
      if (
        ...
        Object.prototype.hasOwnProperty.call(
          this.caseDataLookup,
          this.glidWithPopup + "-" + this.selectedIsoDate
        )
      ) {
        var content = this.caseDataLookup[
          this.glidWithPopup + "-" + this.selectedIsoDate
        ].content;
        for (var i = 0; i < this.dataDiffsArray.length; i++) {
    ...
    showDataUnderClick(obj) {
      ...
      if (this.lastMapClickLoc !== obj.latlng.lat + "/" + obj.latlng.lng) {
        ...
        fetch(
          "https://api.contactmap.me/glid_for_latlng/" +
            obj.latlng.lat +
            "/" +
            obj.latlng.lng
        )
              ...
              if (data.glids.length > 1) {
                for (var i = 0; i < data.glids.length; i++) {
                  if (
                    Object.prototype.hasOwnProperty.call(
                      me.caseDataLookup,
                      data.glids[i] + "-" + me.selectedIsoDate
                    )
                  ) {
                    if (
                      !Object.prototype.hasOwnProperty.call(
                        me.geoJsonStatusLookup,
                        data.glids[i]
                      )
                    ) {
    ...
    hideOffscreenPolygons() {
      ...
      for (let i = 0; i < this.geojsons.length; i++) {
        if (
          typeof this.geojsons[i] === "undefined" ||
          this.geojsons[i].loaded === false
        ) {
          continue;
        }
      ...
      if (Object.prototype.hasOwnProperty.call(this.$refs, "map")) {
        var bbKey =
          ...
          this.$refs.map.mapObject
            ...
            .join("/");
        if (
          Object.prototype.hasOwnProperty.call(me.dataCache.glidsInView, bbKey)
        ) {
    ...
    hideHiddenPolygons() {
      ...
      for (var glid in this.geoJsonStatusLookup) {
        if (
          !this.geoJsonStatusLookup[glid].visible &&
          this.polygonsInView.indexOf(glid) > -1
        ) {
      ...
      fetch("https://api.contactmap.me/geometry/" + geotable + "/" + geoid)
        .then(function(response) {
          ...
          response.json().then(function(data) {
            ...
            let content = "No data";
            let latlng = latLng(
              me.locationsLookup[glid].lat,
              me.locationsLookup[glid].lng
            );
            let geojsonObj = {
            ...
            if (
              Object.prototype.hasOwnProperty.call(
                me.caseDataLookup,
                glid + "-" + me.selectedIsoDate
              ) &&
              Object.prototype.hasOwnProperty.call(
                me.caseDataLookup[glid + "-" + me.selectedIsoDate],
                "value"
              )
            ) {
              ...
              colour =
                me.dataColorLookup[
                  me.caseDataLookup[glid + "-" + me.selectedIsoDate].colourIndex
                ];
              content =
                me.caseDataLookup[glid + "-" + me.selectedIsoDate].content;
              geojsonObj.style = {
                fillColor: colour,
                color: colour,
                weight: 0,
    ...
    updatePolygons() {
      ...
      for (let glid in this.geoJsonStatusLookup) {
        if (
          !this.geoJsonStatusLookup[glid].loading &&
          !this.geoJsonStatusLookup[glid].visible &&
          Object.prototype.hasOwnProperty.call(
            this.geoJsonStatusLookup[glid],
            "geojsonObj"
          )
        ) {
          //console.log("pushing into geojsons "+JSON.stringify(this.geoJsonStatusLookup[glid].geojsonObj));
      ...
      for (let i = 0; i < this.geojsons.length; i++) {
        ...
        if (
          ...
          if (
            Object.prototype.hasOwnProperty.call(
              this.caseDataLookup[glid + "-" + this.selectedIsoDate],
              "value"
            )
          ) {
            value = this.caseDataLookup[glid + "-" + this.selectedIsoDate]
              .value;
            // colour = value >= this.dataColorLookup.length ? this.dataColorLookup[this.dataColorLookup.length - 1] : this.dataColorLookup[value];
            colour = this.dataColorLookup[
              this.caseDataLookup[glid + "-" + this.selectedIsoDate].colourIndex
            ];
          }
          // console.log("pushing into newArray "+JSON.stringify(this.geojsons[i]));
          let geojsonObj = {
            ...
            value: value,
            content: this.caseDataLookup[glid + "-" + this.selectedIsoDate]
              .content,
      ...
      for (var glid in this.geoJsonStatusLookup) {
        if (this.geoJsonStatusLookup[glid].visible) {
          if (
            ...
            if (
              this.caseDataLookup[glid + "-" + to].value -
                this.caseDataLookup[glid + "-" + from].value !==
              0
            ) {
            ...
            console.log(
              "didn't have data for " + glid + " from " + from + " and " + to
            );
          }
        }
      }
    ...
    getPlaceData() {
      if (
        Object.prototype.hasOwnProperty.call(
          this.dataCache.places,
          this.selectedState.value.name + "/" + this.selectedDate.value
        )
      ) {
        console.log(
          "** place data retrieved from dataCache for " +
            ...
            for (var key in data) {
              if (
                ...
                Vue.set(me.caseDataLookup, key + "-" + me.selectedIsoDate, {
                  ...
                  colourIndex: Math.ceil(
                    (data[key].cases / data.highest_cases) * 239
                  ),
                  value: data[key].cases.toString(),
                  content:
                    data[key].cases.toString() +
                    " Covid-19 Cases<br>" +
                    data[key].place +
                    " " +
                    data[key].state,
    ...
    getPostcodeData() {
      if (
        Object.prototype.hasOwnProperty.call(
          this.dataCache.postcodes,
          this.selectedState.value.name + "/" + this.selectedDate.value
        )
      ) {
        console.log(
          "** postcode data retrieved from dataCache for " +
            this.selectedState.value.name +
            "/" +
            this.selectedDate.value
        );
    ...
    if ("geolocation" in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        err => {
          this.gettingLocation = false;
          console.log(
            "Got an error when geolocating, perhaps Safari being annoying..."
          );
          console.log(err);
          setTimeout(function() {
  ...
  mounted() {
    ...
    for (let i = 0; i < 160; i++) {
      this.dataColorLookup.push(
        rgbToHex(255, 240 - Math.ceil((240 / 160) * i), 0)
      );
    }
    // for (let i = 0; i < 80; i++) {
...
function formatDatetime(isoString, justTime) {
  ...
  return (
    dateParts[3] + ":" + dateParts[4] + " " + dateParts[2] + "/" + dateParts[1]
  );
}
