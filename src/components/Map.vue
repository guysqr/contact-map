...
export default {
  ...
  methods: {
    ...
    //   }
    //   // this.filterLocations("updateLayersAfterPanAndZoom");
    // },
    showPopup(obj) {
    ...
    refreshGeojsonPopup() {
      if (
        ...
        this.geoJsonPopup.setContent(
          this.caseDataLookup[this.glidWithPopup].content
        );
    ...
    hideOffscreenPolygons(glids) {
      let newArray = [];
      for (let i = 0; i < this.geojsons.length; i++) {
        if (glids.indexOf(this.geojsons[i].id) > -1) {
          ...
          .then(function(response) {
            ...
            response.json().then(function(data) {
              ...
              if (data.glids.length > 0) {
                me.hideOffscreenPolygons(data.glids);
                for (var i = 0; i < data.glids.length; i++) {
      ...
      fetch("https://api.contactmap.me/geometry/" + geotable + "/" + geoid)
        .then(function(response) {
          ...
          response.json().then(function(data) {
            ...
            if (
              Object.prototype.hasOwnProperty.call(me.caseDataLookup, glid) &&
              Object.prototype.hasOwnProperty.call(
                me.caseDataLookup[glid],
                "value"
              )
            ) {
              value = me.caseDataLookup[glid].value;
              colour =
                value >= me.dataColorLookup.length
                  ? me.dataColorLookup[me.dataColorLookup.length - 1]
                  : me.dataColorLookup[value];
    ...
    updatePolygons() {
      ...
      for (let i = 0; i < this.geojsons.length; i++) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.caseDataLookup, glid)) {
          ...
          if (
            Object.prototype.hasOwnProperty.call(
              this.caseDataLookup[glid],
              "value"
            )
          ) {
            value = this.caseDataLookup[glid].value;
            colour =
              value >= this.dataColorLookup.length
                ? this.dataColorLookup[this.dataColorLookup.length - 1]
                : this.dataColorLookup[value];
          }
          //console.log("pushing into newArray "+JSON.stringify(this.geojsons[i]));
          newArray.push({
    ...
    filterLocations() {
      ...
      if (this.hideHeatmap === false) {
        this.filteredLatLngPostcodeArray.splice(
          0,
          this.filteredLatLngPostcodeArray.length
        );
        for (i = 0; i < this.latLngPostcodeArray.length; i++) {
          if (
            this.$refs.map.mapObject
              ...
              .contains(
                latLng(
                  this.latLngPostcodeArray[i][0],
                  this.latLngPostcodeArray[i][1]
                )
              )
          ) {
        ...
        this.filteredLatLngPlaceArray.splice(
          0,
          this.filteredLatLngPlaceArray.length
        );
        for (i = 0; i < this.latLngPlaceArray.length; i++) {
          if (
            this.$refs.map.mapObject
              .getBounds()
              .contains(
                latLng(this.latLngPlaceArray[i][0], this.latLngPlaceArray[i][1])
              )
          ) {
      ...
      if (this.showMarkers) {
        this.filteredCasePlaceArray.splice(
          0,
          this.filteredCasePlaceArray.length
        );
        for (i = 0; i < this.casePlaceArray.length; i++) {
          if (
            this.$refs.map.mapObject
              .getBounds()
              .contains(this.casePlaceArray[i].latlng)
          ) {
            this.filteredCasePlaceArray.push(this.casePlaceArray[i]);
          }
        }
        this.filteredCasePostcodeArray.splice(
          0,
          this.filteredCasePostcodeArray.length
        );
        for (i = 0; i < this.casePostcodeArray.length; i++) {
          if (
            this.$refs.map.mapObject
              .getBounds()
              .contains(this.casePostcodeArray[i].latlng)
          ) {
  ...
  mounted() {
    ...
    for (let i = 0; i < 30; i++) {
      this.dataColorLookup.push(
        rgbToHex(240, 240 - Math.ceil((240 / 30) * i), 0)
      );
    }
    for (let i = 0; i < 30; i++) {
    ...
    document.addEventListener("POSTCODE_DATA_LOADED", this.showPolygonsInView);
